...
| **Landmark Web** (Pioneer/Catalis) | Page title "Landmark Web …"; search tiles calling `LaunchDisclaimer(…)`; "Property Fraud Alert" link; "Subscriber Log On"; base path `/LandmarkWeb`, `/Landmark`, or domain root | This skill (`references/landmark.md`) — **regime is per-deployment**: FL (Palm Beach) has an on-form **reCAPTCHA** (one human click) + pre-parsed legal columns; **GA (DeKalb) is fully open, no reCAPTCHA**, with labeled legals embedding a direct parcel. Possible Akamai interstitial (Lee) |
